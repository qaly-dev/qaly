...
The fastest path is the wizard, which detects adb/emulator, writes config, and
can register the MCP server:

```bash
qaly init        # interactive setup
qaly doctor      # read-only: check your environment without changing anything
```
