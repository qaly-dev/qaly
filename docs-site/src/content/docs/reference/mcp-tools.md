...
Sends a key event: `"back"`, `"home"`, `"enter"`, `"recent"`.
...
### `run_tests(test_file, filter?, workers?, headless?, avd_name?, auto_heal?, debug?, skip_unrecorded?)`

Replays all tests in a file. `workers > 1` runs in parallel, spawning extra
emulators automatically (`headless`, `avd_name` control them). Unrecorded tests
are recorded automatically unless `skip_unrecorded: true`.
